...
import com.example.zunnorain.noque.AdapterClasses.RescheduleAppointmentListAdapter;
...
public class RescheduleAppointment extends AppCompatActivity implements View.OnClickListener {
    ...
    private RescheduleAppointmentListAdapter adapter;
    private RecyclerView recyclerView;
    private String baseURL, baseURL2;

    private String doctorID;
    private int preDay, preStartTime, preEndTime;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new RescheduleAppointmentListAdapter(getApplicationContext(), list, new RescheduleAppointmentListAdapter.MyAdapterListener() {
            @Override
    ...
    private void checkAppointmentStatus(int position, boolean status, int s, int s1) {

        if (position == preDay - 1 && preStartTime == s) {
            AlertDialog.Builder builder1 = new AlertDialog.Builder(this);
            builder1.setTitle("Make Appointment");
            builder1.setMessage("Select Different slot for schedule");
            ...
            updateAppointment(position + 1, s, s1);
    }

    private void updateAppointment(int i, int s, int s1) {
        ...
        baseURL2 = getString(R.string.baseURL) + "updateAppointment";
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, baseURL2, jsonObject, new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                ...
                try {
                    ...
                    if (s.equals("true")) {
                        Toast.makeText(RescheduleAppointment.this, response.getString("message"), Toast.LENGTH_SHORT).show();
                        finish();
                    }
                } catch (Exception e) {
    ...
    private void sendRequest() {
        ...
        baseURL = getString(R.string.baseURL) + "getReSchedule?token=" + session.getKeyToken();
        list.clear();
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    if (s.equals("true")) {
                        ...
                        String meassage = response.getString("message");
                        if (meassage.equals("No Appointments")) {
                            mainLayout.setVisibility(View.INVISIBLE);
                            progressBar.setVisibility(View.INVISIBLE);
                            messageText.setVisibility(View.VISIBLE);
                            messageText.setText("You Don't have any Appointment");
                        } else {
                            ...
                            JSONObject preApp = response.getJSONObject("current_app");
                            preDay = Integer.parseInt(preApp.getString("day"));
                            preStartTime = Integer.parseInt(preApp.getString("startTime"));
                            preEndTime = Integer.parseInt(preApp.getString("endTime"));
                            ...
                            if (preStartTime == 9)
                                list.get(preDay - 1).setSlotOneBackground(R.drawable.time_interval_item_blue);
                            else if (preStartTime == 10)
                                list.get(preDay - 1).setSlotTwoBackground(R.drawable.time_interval_item_blue);
                            else if (preStartTime == 11)
                                list.get(preDay - 1).setSlotThreeBackground(R.drawable.time_interval_item_blue);
                            else if (preStartTime == 12)
                                list.get(preDay - 1).setSlotFourBackground(R.drawable.time_interval_item_blue);
                            else if (preStartTime == 13)
                                list.get(preDay - 1).setSlotFiveBackground(R.drawable.time_interval_item_blue);
                            else if (preStartTime == 14)
                                list.get(preDay - 1).setSlotSixBackground(R.drawable.time_interval_item_blue);
                            else if (preStartTime == 15)
                                list.get(preDay - 1).setSlotSevenBackground(R.drawable.time_interval_item_blue);
